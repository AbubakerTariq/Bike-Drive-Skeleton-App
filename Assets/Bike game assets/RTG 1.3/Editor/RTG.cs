...
public class RTG : EditorWindow {
	...
	static void Init ()
    {
        RTG window = (RTG)GetWindow(typeof (RTG),false, "RTG 1.3.7", true);
		window.Show();
	}
    ...
    void OnGUI ()
    {
        if (!trackGenerator)
        {
			trackGenerator = AssetDatabase.LoadAssetAtPath("Assets/Bike game assets/RTG 1.3/TrackGenerator.prefab", typeof(GameObject)) as GameObject;
		}
    ...
    private void Combine2(GameObject _objs, GameObject _Objects, string path, int idx, bool save)
    {
        ...
        SerializedObject Sob = new(GameObject.Find("Trees").GetComponent<Renderer>());
        SerializedProperty Sprop = Sob.FindProperty("m_LightmapParameters");
        ...
        foreach (DictionaryEntry mtm in materialToMesh)
        {
            ...
            {
                SerializedObject Sob2 = new(go.GetComponent<Renderer>());
                SerializedProperty Sprop2 = Sob2.FindProperty("m_LightmapParameters");
    ...
    private Component[] GetMeshFilters(GameObject objs)
    {
        ...
        Component[] temp = objs.GetComponentsInChildren(typeof(MeshFilter));
