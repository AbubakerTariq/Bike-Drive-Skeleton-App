...
public class Track_Generator : MonoBehaviour
{
    ...
    [HideInInspector] public GameObject screenCameraPrefab;
    [HideInInspector] public GameObject AudienceSound;
    ...
    private bool SetContainer()
    {
        ...
        if (!tContainer)
        {
            ...
            if (tContainer && tContainer.GetComponent<Track_Config>())
            {
            ...
            if (FindObjectOfType<Track_Config>())
                tContainer = FindObjectOfType<Track_Config>().gameObject;
    ...
    public bool CreateTrack(bool enableAudience, bool audioAudience, int _size)
    {
        ...
        cameraBigScreen = Instantiate(screenCameraPrefab, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0));
        ...
        if (enableAudience == false)
            audioAudience = false;

        if (audioAudience)
            AddSoundAudience(audioAudience);
    ...
    public void ChangeBackGround(Transform container)
    {
        ...
        if (backgrounds.Length > 0)
        {
            ...
            bk = Instantiate(backgrounds[idx_mb], new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0));
            bk.name = "_Background";
            ...
            if (pai)
                bk.transform.SetParent(pai.transform);
        }
    }
    ...
    private int CreateModuloNew(int bloco, int tipo, int orientacao, int tipoBegin, int tipoEnd, bool pit)
    {
        ...
        if (pit)
        {
            ...
            NextType = tipoBegin;
            m = track.Grid22[idx];
            md = Instantiate(m, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0));
    ...
    public GameObject ChangeSection2(Transform section, bool nextSection)
    {
        ...
        GameObject _modulo;
        _modulo = Instantiate(GP, new Vector3(0, 0, 0), Quaternion.Euler(0, 0, 0));
    ...
    {
        GameObject sAud;
        GameObject[] objs = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name == "Audience").ToArray();
        int n = objs.Length;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                sAud = Instantiate(AudienceSound, new Vector3(0, 5, 12), Quaternion.Euler(0, 0, 0));
    ...
    {
        GameObject[] audienceObjs = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name == "Audience").ToArray();
    ...
    public void ChangeTextureRoad(int qualMaterial, GameObject GmObj, int forceOld)
    {
        ...
        GameObject[] myTracks = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name == "Road" && g.transform.parent.parent.parent == GmObj.transform).ToArray();
        Material[] mats;
    ...
    public void ChangeTextureGrass(int qual, GameObject GmObj, int forceOld)
    {
        ...
        GameObject[] myTracks = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name == "Grass" && g.transform.parent.parent.parent == GmObj.transform).ToArray();
        Material[] mats;
    ...
    public void ChangeTextureFenceBase(int qualMaterial, GameObject GmObj, int forceOld)
    {
        ...
        GameObject[] myTracks = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name == "Track" && g.transform.parent.parent.parent == GmObj.transform).ToArray();
        Material[] mats;
