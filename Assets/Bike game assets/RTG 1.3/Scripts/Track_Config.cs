...
public class Track_Config : MonoBehaviour
{
    ...
    [HideInInspector] public int[] _tipo;
    [HideInInspector] public int[] _begin;
    [HideInInspector] public int[] _end;
    [HideInInspector] public int[] _idx;
    [HideInInspector] public int indexMaterialGrass = 0;
    [HideInInspector] public int indexMaterialRoad = 0;
    [HideInInspector] public int indexMaterialFences = 0;
    ...
    void Awake()
    {
        ...
        if (mobile)
        {
            GameObject[] disabledOnMobile = FindObjectsOfType(typeof(GameObject)).Select(g => g as GameObject).Where(g => g.name.Equals("AutoDisabledOnMobile")).ToArray();
